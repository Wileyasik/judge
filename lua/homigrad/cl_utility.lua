...
--\\ Custom tinnitus sound (server->client)
	if CLIENT then
		local CustomTinnitusStation
		local function PlayCustomTinnitusFile(snd)
			if not snd or snd == "" then return end
			if IsValid(CustomTinnitusStation) then
				CustomTinnitusStation:Stop()
				CustomTinnitusStation = nil
			end
			sound.PlayFile("sound/" .. snd, "noblock noplay", function(station)
				if not IsValid(station) then return end
				CustomTinnitusStation = station
				station:SetVolume(1)
				station:Play()
			end)
		end
		local function StopCustomTinnitusFile()
			if not IsValid(CustomTinnitusStation) then return end
			CustomTinnitusStation:Stop()
			CustomTinnitusStation = nil
		end
		net.Receive("hg_play_client_sound_file", function()
			PlayCustomTinnitusFile(net.ReadString())
		end)
		net.Receive("hg_stop_custom_tinnitus", StopCustomTinnitusFile)
	end
--//
