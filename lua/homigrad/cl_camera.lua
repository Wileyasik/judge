...
local hg_fov = ConVarExists("hg_fov") and GetConVar("hg_fov") or CreateClientConVar("hg_fov", "70", true, false, "Change first-person field of view", 75, 115)
...
CalcView = function(ply, origin, angles, fov, znear, zfar)
	...
	view.znear = 1 -- 3
	view.zfar = zfar
	view.fov = math.Clamp(hg_fov:GetFloat(),75,115) + fova[1] + lerpfovadd + lerpfovadd2 - depFovAdd
...
local function renderscene(pos, angle, fov)
	...
	renderView.w = scrw
	renderView.h = scrh
	renderView.fov = isnumber(view.fov) and view.fov or fov
	...
	renderingScene = true
	RENDERSCENE = true
	local ok, err = pcall(render_RenderView, renderView)
	RENDERSCENE = nil
