-- powerbomb
local CFG = {
    ANIM_ATTACKER = "powerbomb_zero",
    ANIM_TARGET   = "powerbomb_one",
    COOLDOWN      = 5,
}

WWE.RegisterMove("powerbomb", {
    ...
    Execute = function(ply, target)
        ...
        if not aId or aId < 0 or aDur <= 0 or not tId or tId < 0 or tDur <= 0 then
            ply:ChatPrint("[WWE] Powerbomb animations are not loaded on this model.")
            return false
        end
        ...
        local ragT = WWE.RagdollDown(target, target:GetModel(), CFG.ANIM_TARGET,   dur, { pos = originPos + Vector(0, 0, 5), angles = originAng })
        ragT.WWE_FakeVel = Vector(351, 0, 0)
        timer.Simple(dur-0.3, function()
            WWE.StandUp(ply)
            ragT.WWE_FakeVel = Vector(0, 0, 0)
            end)
        timer.Simple(math.max(0.2, dur - 0.3), function() WWE.ApplyMoveDamage(target, ply, 70) end)
